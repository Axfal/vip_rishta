...
class ForgotPasswordBloc
  ...
  Future<void> _onEmailChange(
    ...
    emit(
      state.copyWith(email: event.email, apiResponse: ApiResponse.initial()),
    );
  }

  Future<void> _onOTPChange(
    OTPChangeEvent event,
    Emitter<ForgotPasswordState> emit,
  ) async {
    emit(state.copyWith(otp: event.otp, apiResponse: ApiResponse.initial()));
  }
  ...
  Future<void> _onRequestOTP(
    ...
    try {
      final data = {"otp_type": "login_otp", "target": state.email};
      ...
      if (response != null && response['success'] == true) {
        ...
        emit(
          state.copyWith(apiResponse: ApiResponse.error("Failed to send OTP")),
        );
      }
    } catch (e) {
